...
find_damage_property <- function(first_date = NULL, last_date = NULL, ts_only = FALSE,
  ...
  if(!is.null(first_date) & !is.null(last_date)){
    ...
    if(last_date < first_date | lubridate::year(first_date) != lubridate::year(last_date)){
  ...
  if(!is.null(first_date) & !is.null(last_date)){
    storm_data <- storm_data %>%
      ...
      dplyr::filter(!is.na(begin_date) &
                      begin_date %within% lubridate::interval(lubridate::ymd(first_date), lubridate::ymd(last_date))) %>%
