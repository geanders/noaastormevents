...
map_directdeaths <- function(data, begin_date, end_date, storm = FALSE, track = FALSE, dist_limit = NA){
  ...
  DirectDea = rbind(region, DirectDea)

  aggDirectDea = aggregate(value ~ region, data = DirectDea, sum)
  aggDirectDea[, 2] <- ifelse(aggDirectDea[ ,2] == 0, NA, aggDirectDea[ ,2])
  ...
  out <- choroplethr::CountyChoropleth$new(aggDirectDea)
