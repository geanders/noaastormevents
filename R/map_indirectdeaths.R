...
map_indirectdeaths <- function(data, begin_date, end_date, storm = FALSE, track = FALSE, dist_limit = NA){
  ...
  IndirectDea = data1[,c("FIPS","DEATHS_INDIRECT")]
  ...
  IndirectDea = rbind(region, IndirectDea)

  aggIndirectDea = aggregate(value ~ region, data = IndirectDea, sum)
  aggIndirectDea[, 2] <- ifelse(aggIndirectDea[ ,2] == 0, NA, aggIndirectDea[ ,2])
  ...
  out <- choroplethr::CountyChoropleth$new(aggIndirectDea)
