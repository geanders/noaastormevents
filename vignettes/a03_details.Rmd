...
times_translation <- tibble(times = c("K", "M", "G"),
                            times_num = c(10^3, 10^6, 10^9))


storm_file_page <- read_html(url)
storm_file_page %>% 
  html_node("table") %>% 
...
events_2015 %>%
  ...
  geom_polygon(data = map_data("state", region = "arkansas"),
               aes(x = long, y = lat, group = group),
                   color = "black", fill = NA) + 
  geom_segment(aes(xend = END_LON, yend = END_LAT), 
...
Within the code in `noaastormevents`, the `match_forecast_county` is used to try to match a county FIPS to each of the events listed by forecast zone. To get the full code for that function, you can run `match_forecast_county` (i.e., the function name, without parentheses after). To match an events listed by forecast zone to a county, this function tries the following to try to `cz_fips` and `cz_name` columns in the storm events data to the state and county fips codes in the `forecast_zone_data` dataframe sourced from the National Weather Service Public Forecast Zones.: 

1. Tries to match `cz_fips` to the county code in `ZONE` and `state_fips` to `state_code`.
2. Next, for all remaining events, try to match `cz_name` to `cz_name` and `state_fips` to `state_code`.

It is important to note that many forecast zones span multiple counties. As a result, some storm events will have multiple fips code matches. This may make your resulting dataframe longer, but still accurate. Generally, the events in these forecast zones are generally larger, applying to the majority of the zone. 
...
In the 2015 events data, there are 22,790 events listed by forecast zone rather than county. Once the `match_forecast_county` function is applied to these events, 19,127 events were linked to at least one county (84%). Many forecast zones cover multiple counties, and therefore some events are linked to multiple counties. This results in 24,426 total event/county pairings.

3,633 events (16%) could not be matched to a county.
...
outside_events <- z_events_2015 %>%
  filter(state %in% c("GULF OF MEXICO", "GUAM", "ATLANTIC NORTH",
                                    ...
                                    "LAKE SUPERIOR", "ATLANTIC SOUTH", "LAKE MICHIGAN",
                                    "HAWAII WATERS", "PUERTO RICO", "E PACIFIC", "LAKE ERIE",
                                    "LAKE ONTARIO", "VIRGIN ISLANDS", "ALASKA")) %>%
  filter(is.na(forecastfips)) 
```
...
This left 636 events in the continental U.S. that were listed by forecast zone but could not be linked to a county by the `match_forecast_county` function. 
...
test <- z_events_2015 %>%
  ...
  dplyr::summarize(value = n()) %>%
  ungroup() %>%
  left_join(maps::county.fips, by = c("FIPS" = "fips")) %>%
  left_join(unite(map_data("county"), polyname, region, subregion, sep = ","),
            by = "polyname") %>%
  ggplot(aes(x = long, y = lat, group = group, fill = value)) +
  geom_polygon(color = NA) + 
...
This function uses the forecast zone data sourced in September 2023. There is no public data on historic forecast zones. The NWS updates zones on a non-consistent basis, often only changing a few zones at a time. As a result, some older storm event data may have a lower matching rate than data from recent years. An older version of this function is available in `old_match_forecast_county`. This function uses `cz_name` and takes several steps to extract a match to the `county.fips` package available in the `maps` package. If you are not getting a high enough match rate with the `match_forecast_county`, it is recommended to try the older version and see if it returns more matches.
...
c_events_2015 <- events_2015 %>%
  dplyr::select(.data$BEGIN_YEARMONTH, .data$BEGIN_DAY, .data$END_YEARMONTH, .data$END_DAY,
                  ...
                  .data$DEATHS_DIRECT, .data$DEATHS_INDIRECT, .data$INJURIES_DIRECT,
                  .data$INJURIES_INDIRECT, .data$DAMAGE_PROPERTY, .data$DAMAGE_CROPS,
  ...
  dplyr::mutate(fips = as.numeric(paste0(.data$state_fips, str_pad(.data$cz_fips, 3, pad = "0"))))
...
```{r fig.height = 10, fig.width = 7, fig.align = "center", echo = FALSE, message = FALSE, warning = FALSE}
library(maps)
data(state.fips)
