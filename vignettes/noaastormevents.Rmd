...
save(floyd_events, file = "data/floyd_events.RData")
```
```{r echo = 2}
load("data/floyd_events.RData")
head(floyd_events)
```
...
```{r}
map_events(storm = "Floyd-1999", dist_limit = 300,
           east_only = FALSE)
```
...
```{r}
map_events(storm = "Floyd-1999", dist_limit = 300, add_tracks = TRUE,
           plot_type = "number of events")
```
