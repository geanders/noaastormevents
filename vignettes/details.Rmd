...
This raw data has `r ncol(events_2015)` columns. This includes:
...
- unique IDs for the event and the epidsode, which can be used to link listings for events and episodes across the three types of files ("details", "locations", and "fatalities") (`EPISODE_ID`, `EVENT_ID`). Note that there are more unique event IDs (`r prettyNum(length(unique(events_2015$EVENT_ID)), big.mark = ",")` for the 2015 events data) than unique episode IDs (`r prettyNum(length(unique(events_2015$EPISODE_ID)), big.mark = ",")` for the 2015 events data)
...
In the 2015 events data, there are `r prettyNum(nrow(z_events_2015), big.mark = ",")` events listed by forecast zone rather than county. Once the `match_forecast_county` function is applied to these events, `r prettyNum(sum(!is.na(z_events_2015$fips)), big.mark = ",")` events were linked to a county (`r round(100 * mean(!is.na(z_events_2015$fips)))`%) while `r prettyNum(sum(is.na(z_events_2015$fips)), big.mark = ",")` events (`r round(100 * mean(is.na(z_events_2015$fips)))`%) could not be matched to a county.
...
Of the events not matched to a county, `r prettyNum(nrow(outside_events), big.mark = ",")` events were outside the continental U.S. (i.e., in Hawaii, Alaska, U.S. territories, or waters): 
...
For the events listed by forecast zone that could be successfully matched to a county
...
When using this function, and for event listings generated using this function, the user may want to hand-check that event lisings with names like "Lake" and "Mountain" in the `cz_name` column are not erroneously matched to counties with names like "Lake County" and "Mountain County". Code like the following can be used for these checks (in this case, checking a dataframe of event listings named `z_events_2015` that is output from `match_forecast_county` and so has `fips` added for each event listing, if a match could be found): 
...
county_damage_2015 <- county_events_2015 %>%
  ...
  dplyr::mutate(amount_times = stringr::str_extract(amount, "[A-Z]"),
                amount_times = stringr::str_to_upper(amount_times),
                amount_times = ifelse(amount_times == "K", "1000", amount_times),
                amount_times = ifelse(amount_times == "M", "1000000", amount_times),
                amount_times = ifelse(amount_times == "B", "1000000000", amount_times),  
                amount_times = ifelse(amount_times == "T", "1000000000000", amount_times),
                amount_times = as.numeric(amount_times),
                amount = stringr::str_extract(amount, "[0-9.]*+"),
                amount = as.numeric(amount),
                amount = ifelse(!is.na(amount_times), amount * amount_times, amount),
                amount = ifelse(is.na(amount), 0, amount)) %>%
  dplyr::select(- amount_times)
```
...
Of the times when a county had two or more events as part of the same episode in 2015, in `r formatC(a[a$zero_range == FALSE, ]$n, big.mark = ",")` (`r round(100 * a[a$zero_range == FALSE, ]$n / (sum(a[ , "n"])))`%) cases the damage costs were not identical across the event listings for the county and episode, while in `r formatC(a[a$zero_range == TRUE, ]$n, big.mark = ",")` cases (`r round(100 * a[a$zero_range == TRUE, ]$n / (sum(a[ , "n"])))`%) the cost listings were identical across event listings (as shown in the cases just given). If using damage cost estimates for research, it may be wise to hand-check for cases where damage cost estimates are duplicated across different events in a way that prevents summing across events to get the cumulative episode total cost. 
